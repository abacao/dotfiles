...
case "$1" in
	...
	--select|-s)
		...
		scrot -s /tmp/screenshot-$(date +%F_%T).png -e 'xclip -selection c -t image/png < $f' -e 'feh $f &' &&
		sleep 1 && notify-send "screenshot has been saved in /tmp/screenshot-$(date +%F_%T).png"
		;;
	--help|-h)
		echo "
available options:
